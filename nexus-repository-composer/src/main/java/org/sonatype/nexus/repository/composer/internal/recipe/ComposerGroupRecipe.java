...
@Singleton
public class ComposerGroupRecipe
    extends ComposerRecipeSupport
{
  public static final String NAME = "composer-group";

  @Inject
  private Provider<GroupFacetImpl> groupFacet;

  @Inject
  private GroupHandler standardGroupHandler;

  @Inject
  private ComposerGroupPackagesJsonHandler packagesJsonHandler;

  @Inject
  private ComposerGroupProviderJsonHandler providerJsonHandler;

  @Inject
  private ComposerGroupPackageJsonHandler packageJsonHandler;

  @Inject
  public ComposerGroupRecipe(@Named(GroupType.NAME) final Type type, @Named(ComposerFormat.NAME) final Format format) {
    super(type, format);
  }
  ...
  private ViewFacet configure(final ConfigurableViewFacet facet) {
    ...
    builder.route(packagesMatcher()
        ...
        .handler(assetKindHandler(AssetKind.PACKAGES))
    ...
    builder.route(providerMatcher()
        .handler(timingHandler)
        .handler(assetKindHandler(AssetKind.PROVIDER))
        .handler(securityHandler)
        .handler(exceptionHandler)
        .handler(handlerContributor)
    ...
    builder.route(packageMatcher()
            .handler(timingHandler)
            .handler(assetKindHandler(AssetKind.PACKAGE))
            .handler(securityHandler)
            .handler(exceptionHandler)
            .handler(handlerContributor)
    ...
    builder.route(zipballMatcher()
        .handler(timingHandler)
        .handler(assetKindHandler(AssetKind.ZIPBALL))
        .handler(securityHandler)
        .handler(exceptionHandler)
        .handler(handlerContributor)
