    ...
    extends ComposerRecipeSupport
{
  ...
  @Inject
  private Provider<ComposerHostedFacet> hostedFacet;

  @Inject
  private LastDownloadedHandler lastDownloadedHandler;

  @Inject
  private ComposerHostedUploadHandler uploadHandler;

  @Inject
  private ComposerHostedDownloadHandler downloadHandler;

  @Inject
  public ComposerHostedRecipe(@Named(HostedType.NAME) final Type type, @Named(ComposerFormat.NAME) final Format format) {
    super(type, format);
  }
  ...
  private ViewFacet configure(final ConfigurableViewFacet facet) {
    ...
    builder.route(packagesMatcher()
        ...
        .handler(assetKindHandler(PACKAGES))
    ...
    builder.route(providerMatcher()
        .handler(timingHandler)
        .handler(assetKindHandler(PROVIDER))
        .handler(securityHandler)
        .handler(exceptionHandler)
        .handler(handlerContributor)
        .handler(conditionalRequestHandler)
    ...
    builder.route(packageMatcher()
        .handler(timingHandler)
        .handler(assetKindHandler(PACKAGE))
        .handler(securityHandler)
        .handler(exceptionHandler)
        .handler(handlerContributor)
        .handler(conditionalRequestHandler)
    ...
    builder.route(zipballMatcher()
        .handler(timingHandler)
        .handler(assetKindHandler(ZIPBALL))
        .handler(securityHandler)
        .handler(exceptionHandler)
        .handler(handlerContributor)
        .handler(conditionalRequestHandler)
